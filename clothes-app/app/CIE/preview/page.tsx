...
import PatternPreviewWithColorants from '@/components/PatternPreview';
...
async function extractMainColorsFromImage(file: File, colorCount = 3): Promise<[number, number, number][]> {
...
export default function PreviewPage() {
  ...
  const [extractedBaseColors, setExtractedBaseColors] = React.useState<[number, number, number][]>([]);
  ...
  // Extract main colors from the pattern image
    React.useEffect(() => {
        if (selectedPattern) {
            extractMainColorsFromImage(selectedPattern, designColorants.length || 3).then(setExtractedBaseColors);
        }
  ...
  if (!selectedPattern || colorants.length === 0 || baseColors.length === 0) {
    return (
      <div>
        Please select a pattern and colorants first.<br />
        Base colors length is {baseColors.length}<br />
        Colorants length is {colorants.length}<br />
        Selected pattern: {selectedPattern ? selectedPattern.name : 'None'}<br />
        DesignColorants is {designColorants.length}<br />
        Selected database: {selectedDatabase ? selectedDatabase.name : 'None'}<br />
        DB length: {db.length}
      </div>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-neutral-950 text-white font-sans">
      {/* Hero Section */}
      <section
        className="flex flex-col items-center justify-center text-center py-3 px-6 bg-gradient-to-b from-neutral-900 to-black relative overflow-hidden"
        style={{
      ...
      {/* Preview Section */}
    <section className="flex flex-col items-center justify-center py-0 px-0 bg-neutral-900">
        <div className="bg-neutral-800 rounded-xl p-2 shadow flex flex-col items-center w-full max-w-lg">
            <PatternPreviewWithColorants
